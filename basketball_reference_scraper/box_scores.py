...
try:
    ...
    from basketball_reference_scraper.utils import (
        get_game_suffix,
        remove_accents,
    )
...
def get_box_scores(date, team1, team2, period="GAME", stat_type="BASIC"):
    ...
    r1 = get(
        f"https://widgets.sports-reference.com/wg.fcgi?css=1&site=bbr&url={suffix}&div=div_box-{team1}-{period.lower()}-{stat_type.lower()}"
    )
    r2 = get(
        f"https://widgets.sports-reference.com/wg.fcgi?css=1&site=bbr&url={suffix}&div=div_box-{team2}-{period.lower()}-{stat_type.lower()}"
    )
    dfs = []
...
def get_all_star_box_score(year: int):
    ...
    if r.status_code == 200:
        ...
        team_names = list(
            map(
                lambda el: el.text,
                soup.select("div.section_heading > h2")[1:3],
            )
        )
        ...
        for i, dnp in enumerate(
            map(
                lambda el: el.text, soup.select("ul.page_index > li > div > a")
            )
        ):
            ...
            if dnp not in res[as_team]["PLAYER"].values:
                ...
                team = (
                    stats_df[
                        stats_df["SEASON"] == f"{year-1}-{str(year)[-2:]}"
                    ]
                    .head(1)["TEAM"]
                    .values[0]
                )
